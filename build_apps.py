...
APPS = [
    ("ai-carbon-footprint",    "Carbon Footprint Analyzer",        "145,55,45",   "green",   "Enter your daily activities, travel, diet, and energy use for detailed analysis"),
    ...
    ("ai-net-zero-roadmap",    "Corporate Net-Zero Builder",        "215,45,55",   "blue",    "Build a comprehensive corporate net-zero roadmap with emissions targets and timelines"),
...
def run(cmd, cwd=None, env=None, timeout=120):
    result = subprocess.run(cmd, shell=True, cwd=cwd, capture_output=True, text=True, timeout=timeout, env=env)
    return result.stdout, result.stderr, result.returncode

for app, title, hsl, color, desc in APPS:
    print(f"\n{'='*50}")
    print(f"Building: {app}")
    print(f"{'='*50}")
    
    # Remove existing directory
    run(f"rm -rf {app}", cwd=WORKDIR)
    
    # Create Next.js app
    print("Creating Next.js app...")
    out, err, rc = run(
        f"npx create-next-app@latest {app} --typescript --tailwind --eslint --app --src-dir --import-alias '@/*' --no-turbopack --yes",
        cwd=WORKDIR, timeout=120
    )
    print(out.split('\n')[-3] if out else err[-200:])
    ...
    # Create page.tsx
    page_content = PAGE_TSX.replace("{TITLE}", title).replace("{DESC}", desc).replace("{COLOR}", color)
    ...
    # Try to create repo, if exists just push
    out, err, rc = run(f"gh repo create NebulaLumino/{app} --public --source=. --push 2>&1", cwd=f"{WORKDIR}/{app}", timeout=30)
    if "already exists" in err or "already exists" in out:
        ...
        if rc == 0:
            ...
            print(f"Push: {err[-200:]}")
    elif rc == 0:
        print("Repo created & pushed")
    else:
        print(f"Repo: {err[-200:]}")
