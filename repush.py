...
"""
    ...
    print(f"  Build: {'OK' if rc == 0 else 'ERROR'}")
    ...
    # Try gh create first, if exists use git push
    ...
    if "already exists" in out or "already exists" in err:
        # Repo exists, push with git directly
        run(f"git remote set-url origin https://github.com/NebulaLumino/{app}.git", cwd=f"{WORKDIR}/{app}")
        out, err, rc = run(f"git push -u origin main --force", cwd=f"{WORKDIR}/{app}", timeout=30)
        if rc == 0:
            print(f"  Pushed via git!")
        else:
            print(f"  Git push error: {err[-300:]}")
    elif rc == 0:
        print(f"  Pushed via gh!")
    else:
        print(f"  gh error: {err[-200:]}")
